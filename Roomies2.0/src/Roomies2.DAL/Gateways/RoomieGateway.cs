...
namespace Roomies2.DAL.Gateways
{
    public class RoomieGateway
    {
        ...
        public async Task<Result<RoomieData>> FindById(int roomieId)
        {
            ...
            {
                var r = await con.QueryFirstOrDefaultAsync<RoomieData>(
        ...
        public async Task<Result<int>> Create(string lastName, string firstName, string phone, int sex,
            DateTime birthDate, string desc, string pic)
        {
            ...
            using (var con = new SqlConnection(_connectionString))
            {
                ...
                if (status == 1)
                    return Result.Failure<int>(Status.BadRequest, "A Roomie with this userBame already exists");
        ...
        public async Task<Result> Update(string lastName, string firstName, string phone, int sex, DateTime birthDate,
